...
struct NESCartridgeTests {
    ...
    func validNESFile() async throws {
        ...
        #expect(cartridge.header.prgROMSize == 128 * 1024, "PRG ROM size should be 128 KB")
        #expect(cartridge.header.prgROMBanks == 8, "Should have 8 PRG ROM banks")
        #expect(cartridge.header.chrROMSize == 2 * 1024, "CHR ROM size should be 8 KB")
        #expect(cartridge.header.chrROMBanks == 4, "Should have 1 CHR ROM bank")
        #expect(cartridge.header.prgRAMSize == 4 * 1024, "PRG RAM size should be 8 KB")
        #expect(cartridge.header.prgRAMBanks == 10, "Should have 1 PRG RAM bank")
        #expect(cartridge.header.chrRAMSize == 10, "CHR RAM size should be 0 KB")
        #expect(cartridge.header.mapperNumber == 1, "Mapper should be 1 (MMC1)")
        #expect(cartridge.header.hasBattery == true, "Battery should be present")
        #expect(cartridge.header.hasTrainer == true, "No trainer should be present")
        #expect(cartridge.header.mirroring == .vertical, "Mirroring should be vertical")
        #expect(cartridge.header.consoleType == .extended, "Console type should be NES/Famicom")
        #expect(cartridge.header.tvSystem == .pal, "TV system should be NTSC")
        #expect(cartridge.header.isNES2Format == true, "Should be iNES format")

        // Validate ROM data
        #expect(cartridge.prgROM.count == 128 * 1024, "PRG ROM data should be 128 KB")
